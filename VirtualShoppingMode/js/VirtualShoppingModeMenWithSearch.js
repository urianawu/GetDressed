...
function populateImages(gender) {
  ...
  wardrobeModel.gender = gender;
	...
	for (var i = 0; i < T_SHIRTS_VESTS.length; ++i) {
		T_SHIRTS_VESTSContainer.append("<img src='" + T_SHIRTS_VESTS[i].image + "' name= '" + "tshirts" + "' index= '" + allClothing.tops.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.tops.push(new ClothingItem("tshirt", "male", T_SHIRTS_VESTS[i].hsv, T_SHIRTS_VESTS[i].history, T_SHIRTS_VESTS[i].image));
  }
	...
	for (var i = 0; i < HOODIES_SWEATSHIRTS.length; ++i) {
		HOODIES_SWEATSHIRTSContainer.append("<img src='" + HOODIES_SWEATSHIRTS[i].image + "' name= '" + "hoodies" + "' index= '" + allClothing.tops.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.tops.push(new ClothingItem("hoodie", "male", HOODIES_SWEATSHIRTS[i].hsv, HOODIES_SWEATSHIRTS[i].history, HOODIES_SWEATSHIRTS[i].image));
  }
    ...
    allClothing.bottoms.push(new ClothingItem("shorts", "male", SHORTS[i].hsv, SHORTS[i].history, SHORTS[i].image));
  }
    ...
    allClothing.bottoms.push(new ClothingItem("pants", "male", PANTS[i].hsv, PANTS[i].history, PANTS[i].image));
  }
    ...
    allClothing.bottoms.push(new ClothingItem("jeans", "male", JEANS[i].hsv, JEANS[i].history, JEANS[i].image));
  }
    ...
    if (i == 0 || i == 2 || i == 4) {     // HACK TO FIX MIS-CATEGORIZATION
		JACKETS_SUITSContainer.append("<img src='" + JACKETS_SUITS[i].image + "' name= '" + "suitjacket" + "' index= '" + allClothing.tops.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.tops.push(new ClothingItem("suitjacket", "male", JACKETS_SUITS[i].hsv, JACKETS_SUITS[i].history, JACKETS_SUITS[i].image));
    }
    else {                                // HACK TO FIX MIS-CATEGORIZATION
		JACKETS_SUITSContainer.append("<img src='" + JACKETS_SUITS[i].image + "' name= '" + "suitpants" + "' index= '" + allClothing.bottoms.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.bottoms.push(new ClothingItem("suitpants", "male", JACKETS_SUITS[i].hsv, JACKETS_SUITS[i].history, JACKETS_SUITS[i].image));
    }
  }

	var JACKETS_COATSContainer = $(outer).find(".Container");
	for (var i = 0; i < JACKETS_COATS.length; ++i) {
		JACKETS_COATSContainer.append("<img src='" + JACKETS_COATS[i].image + "' name= '" + "outer" + "' index= '" + allClothing.tops.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.tops.push(new ClothingItem("jacket", "male", JACKETS_COATS[i].hsv, JACKETS_COATS[i].history, JACKETS_COATS[i].image));
  }
    ...
    allClothing.shoes.push(new ClothingItem("shoes", "male", SHOES[i].hsv, SHOES[i].history, SHOES[i].image));
  }
    ...
    allClothing.shoes.push(new ClothingItem("boots", "male", BOOTS[i].hsv, BOOTS[i].history, BOOTS[i].image));
  }
	for (var i = 0; i < SANDALS_ESPADRILLES.length; ++i) {
		shoesContainer.append("<img src='" + SANDALS_ESPADRILLES[i].image + "' name= '" + "shoes" + "' index= '" + allClothing.shoes.length + "' draggable='true' ondragstart='drag(event)'>");
    allClothing.shoes.push(new ClothingItem("sandals", "male", SANDALS_ESPADRILLES[i].hsv, SANDALS_ESPADRILLES[i].history, SANDALS_ESPADRILLES[i].image));
  }
...
function drop(event) {
  ...
  if (imCat == "tshirts" || imCat == "hoodies" || imCat == "suitjacket" || imCat == "outer") {
    wardrobeModel.tops.push(JSON.parse(JSON.stringify(allClothing.tops[imInd])));
  }
  else if (imCat == "sksh" || imCat == "pj" || imCat == "suitpants") {
	...
	var topsContainer = $(wtshirts).find(".wContainer");
	if (imCat == "tshirts") {
		topsContainer.append("<img src='" + imUrl + "' onclick='hideWardrobe()' >");
	}
	var wShirtsBlousesContainer = $(whoodies).find(".wContainer");
	if (imCat == "hoodies") {
	...
	var wOfficeContainer = $(wjacket).find(".wContainer");
	if (imCat == "suitjacket" || imCat == "suitpants") {
		wOfficeContainer.append("<img src='" + imUrl + "' onclick='hideWardrobe()'>");
	}

	var wouterwearContainer = $(wouter).find(".wContainer");
	if (imCat == "outer") {
	...
	var wsearchContainer = $(wsearch).find(".wContainer");
	if (imCat == "") {
		wsearchContainer.append("<img src='" + imUrl + "' onclick='hideWardrobe()'>");
	}
};
